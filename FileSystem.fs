...
type FSItem =
    | File of Name * Data
    | Folder of Name * FSItem list

// based on weaving a web
type Ctx =
    | Top'
    | Folder' of Name * FSItem list * Ctx * FSItem list
...
let fsUp (zipper: FSZipper): FSZipper =
    match zipper with
    | item, FSCrumb (name, before, after) :: rest -> Folder(name, before @ [ item ] @ after), rest
    | _, [] -> zipper // bad?
